...
export function StandingAutopayCard({ league, bowlerId, savedCards }: Props) {
  ...
  const activate = useMutation({
    mutationFn: () => apiRequest(`/api/financials/leagues/${league.id}/standing-autopay/1/consent`, "POST", {
      commandKey: commandKey("standing-consent"), sourceId: selectedCard, partnerBowlerIds: [],
    }),
  ...
  return <Card data-testid="standing-autopay-card"><CardHeader><CardTitle className="flex items-center justify-between">Weekly automatic payments {active ? <Badge>Enabled</Badge> : <Badge variant="secondary">Off</Badge>}</CardTitle></CardHeader><CardContent className="space-y-3">
    <p className="text-sm text-muted-foreground">At each occurrence cutoff, only your exact remaining roster obligations are collected. Manual payment wins first; double-pay groups are collected as their published group. Payments are always scoped to this bowler.</p>
    {active && !replaceMode ? <><p className="text-sm">Saved payment method and consent version are active.</p><div className="flex flex-wrap gap-2"><Button type="button" variant="outline" disabled={revoke.isPending} onClick={() => revoke.mutate()}>Revoke automatic payments</Button><Button type="button" variant="outline" onClick={() => setReplaceMode(true)}>Replace payment method</Button></div></> : <>
      {savedCards.length === 0 ? <p className="text-sm text-muted-foreground">Save a card before enabling automatic payments.</p> : <><label className="block text-sm">Saved card<select className="mt-1 w-full rounded border bg-background p-2" value={selectedCard} onChange={(event) => setSelectedCard(event.target.value)}><option value="">Select a card</option>{savedCards.map((card) => <option key={card.id} value={card.id}>{card.brand} ending {card.last4}</option>)}</select></label><div className="flex flex-wrap gap-2"><Button type="button" disabled={!selectedCard || activate.isPending} onClick={() => activate.mutate()}>{active ? "Replace payment method" : "Enable weekly automatic payments"}</Button>{active && <Button type="button" variant="ghost" onClick={() => setReplaceMode(false)}>Cancel</Button>}</div></>}
    </>}
  </CardContent></Card>;
}
