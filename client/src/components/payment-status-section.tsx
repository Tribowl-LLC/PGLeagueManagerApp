...
export const PaymentStatusSection: FC<PaymentStatusSectionProps> = ({ league, bowler, weeklyFee }) => {
  ...
  const financials = {
    fullSeasonAmount: rows.reduce((sum, row) => sum + Math.max(0, row.amountMinor - (row.waivedMinor ?? 0)), 0),
    totalDueToDate: rows.filter((row) => row.classification !== "future").reduce((sum, row) => sum + Math.max(0, row.amountMinor - (row.waivedMinor ?? 0)), 0),
    ...
    waivedAmount: rows.reduce((sum, row) => sum + (row.waivedMinor ?? 0), 0),
  };
