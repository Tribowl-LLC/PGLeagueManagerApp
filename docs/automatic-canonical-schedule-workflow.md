...
The schedule read contract exposes group ID, role, paired occurrence ID/date, group state, and revision. The builder remains the editing surface. An authoritative canonical double-pay/cancellation edit supplies `scheduleRevision` and `Idempotency-Key`; the schedule lock, editor command/fingerprint, group revocations, and new groups are one transaction. The editor contract is `canonical-schedule-edit/2`: ordinary builder metadata (name, description, activation visibility, practice time, and catalog fee labels) is persisted in that same transaction and its normal post-commit resync/cache effects still run. Same-count `skipDates` and physical date/time edits are applied automatically by ordinal-to-UUID reconciliation, preserving published occurrence and billing identities where the existing safety checks permit it. Count-changing regeneration, cancellation remapping (including combining cancellation with a physical regeneration), locked location/payment/weekly-fee changes, and financial/provider evidence conflicts are rejected explicitly. An elapsed-start correction is permitted only when every proposed slot is future and the whole league is unplayed, unpaid, and free of operations, preparation, allocations, and explicit locks. Stale revisions return 409; replaying the same idempotency key and payload is side-effect free.
...
Physical date/time edits use the existing in-transaction roster materializer in one batched reschedule pass across the updated occurrences. It reads and locks current responsibilities, obligations, and payment evidence once, then writes versioned replacements in bounded batches while preserving the ordinary first-materialization path; metadata-only edits continue to skip financial rematerialization. A PostgreSQL regression fixture for 30 occurrences and 32 roster positions measures the complete save at 316 pooled queries under a local bound of 500. This is a regression guard, not a production latency SLA, and adds no worker, queue, or other process.
...
5. Exercise a full builder-shaped mixed PATCH (metadata plus schedule), same-count skip/date/time edits (including an elapsed-start correction and DST boundary), zero/one/two double-pay triggers, a cancellation of each group role, stale/idempotent retries, reserved/provider/game/explicit-lock fences, and claim-before-cancel and cancel-before-claim races. Verify canonical schedule reads show the exact group badges and revision snapshots form a contiguous chain.
