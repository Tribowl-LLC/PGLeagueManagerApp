...
Migration `0031_league_paying_lineup_size.sql` adds the nullable league configuration and a database check restricting populated values to 3 or 4. It performs no guessed backfill: only a pre-existing complete active F1 activation supplies its already-explicit immutable lineup size. New league setup requires the value, rollover carries an explicitly populated value, and any other existing league must be edited before its F1 source becomes available. The selected value participates in the source fingerprint and cannot change after complete active F1 evidence exists.
