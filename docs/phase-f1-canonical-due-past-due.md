...
Migration `0031_league_paying_lineup_size.sql` adds the nullable league configuration and a database check restricting populated values to 3 or 4. It deliberately performs no inferred backfill. New league setup requires the value, rollover carries an explicitly populated value, and an existing league must be edited before its F1 source becomes available. The selected value participates in the source fingerprint and cannot change after complete active F1 evidence exists.
