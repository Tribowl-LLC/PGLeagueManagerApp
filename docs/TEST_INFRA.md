...
- **Serial browser journey isolation**: `tests/e2e/email-first-registration-journey.test.ts` runs alone in the `serial-email-browser` project with `per-worker-db-only` and `fileParallelism: false`. The journey creates its own in-process Express app and Playwright browser; serial scheduling prevents concurrent active-organization fixture writes from overlapping this browser flow.
...
- **Concurrent-boot safety on shared test DBs**: With deterministic per-pool naming (Task #722), recycled forks and sibling test-app spawns can boot against a DB another boot is mid-install on. `installDbInvariants()` in `server/db-invariants.ts` therefore wraps its DROP TRIGGER / CREATE TRIGGER pair in the shared transaction-scoped schema-writer advisory lock, so the install is atomic per database, cannot race a checked migration or baseline adoption, and the `trigger "users_role_org_required" ... already exists` race is impossible.
