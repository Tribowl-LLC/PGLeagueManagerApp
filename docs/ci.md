...
| `production-database-migration.yml` | `Migrate production database` | Manual `workflow_dispatch` on `main` only | Requires the exact certified SHA and confirmation phrase, verifies the protected Neon production target, creates a protected restorable branch, verifies the locked live catalog against the checked-in pre-release fingerprint, applies only the explicitly expected checked migrations, and verifies the post-release fingerprint plus an immediate no-op rerun |
