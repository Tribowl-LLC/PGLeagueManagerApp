...
export const f3CollectionPolicies = pgTable("f3_collection_policies", {
  ...
  stateCheck: check("f3_policies_state_check", sql`${table.state} IN (${policyStates}) AND f3_json_array_shape(${table.collectionPoints}, 'occurrence-object') AND (${table.state} <> 'approved' OR (${table.approvedByUserId} IS NOT NULL AND ${table.approvedAt} IS NOT NULL))`),
}));
