...
export const paymentOperationRosterSnapshots = pgTable("payment_operation_roster_snapshots", {
  ...
  /** Immutable direct-partner authorization evidence for interactive v3.
   * Kept nullable so every v2 and standing snapshot remains byte-for-byte
   * compatible with its historical codec. */
  partnerEvidence: jsonb("partner_evidence").$type<unknown[] | null>(),
  ...
  amountCheck: check("payment_operation_roster_snapshots_amount_check", sql`${table.amountMinor} > 0 AND ${table.currency} = 'USD' AND ${table.snapshotVersion} IN (2, 3) AND ${table.snapshotKind} IN (${snapshotKinds}) AND ((${table.snapshotKind} = 'interactive' AND ${table.collectionMode} IS NULL AND ${table.cutoffAt} IS NULL AND ${table.requestKind} IN ('direct', 'order') AND ${table.encryptedSourceId} IS NOT NULL AND ${table.payerBowlerId} IS NOT NULL AND ${table.sourceKind} IN ('new_card', 'saved_card', 'wallet') AND ${table.quoteFingerprint} IS NOT NULL AND ((${table.snapshotVersion} = 2 AND ${table.partnerEvidence} IS NULL) OR (${table.snapshotVersion} = 3 AND ${table.partnerEvidence} IS NOT NULL AND jsonb_typeof(${table.partnerEvidence}) = 'array' AND jsonb_array_length(${table.partnerEvidence}) > 0))) OR (${table.snapshotKind} = 'standing_autopay' AND ${table.snapshotVersion} = 2 AND ${table.collectionMode} IN (${collectionModes}) AND ${table.cutoffAt} IS NOT NULL AND ${table.requestKind} IS NULL AND ${table.encryptedSourceId} IS NULL AND ${table.payerBowlerId} IS NULL AND ${table.sourceKind} IS NULL AND ${table.quoteFingerprint} IS NULL AND ${table.partnerEvidence} IS NULL))`),
  fingerprintCheck: check("payment_operation_roster_snapshots_fingerprint_check", sql`(${table.snapshotVersion} = 2 AND ${table.snapshotFingerprint} ~ '^lv(rosterexec|standingcutoff):v1:[0-9a-f]{64}$') OR (${table.snapshotVersion} = 3 AND ${table.snapshotFingerprint} ~ '^lvpartnerexec:v3:[0-9a-f]{64}$')`),
  quoteFingerprintCheck: check("payment_operation_roster_snapshots_quote_fingerprint_check", sql`(${table.quoteFingerprint} IS NULL AND ${table.snapshotKind} = 'standing_autopay') OR (${table.quoteFingerprint} ~ '^lvrosterquote:v1:[0-9a-f]{64}$' AND ${table.snapshotVersion} = 2 AND ${table.snapshotKind} = 'interactive') OR (${table.quoteFingerprint} ~ '^lvpartnerquote:v3:[0-9a-f]{64}$' AND ${table.snapshotVersion} = 3 AND ${table.snapshotKind} = 'interactive')`),
