...
export async function executeCanonicalAutopayOperation(input: { organizationId: number; operationId: string; now?: Date }): Promise<void> {
  ...
  if (!currentCredentials?.success || currentCredentials.data?.locationId !== snapshot.providerLocationId) { await recordPaymentOperationFailedTerminal({ organizationId: input.organizationId, operationId: operation.id, leaseToken: operation.leaseToken, errorClassification: "invalid_request", errorCode: "F4_PROVIDER_LOCATION_DRIFT", now }); return; }
  if (operation.leagueId === null || !(await acquireCanonicalAutopayDispatchCutoff({ organizationId: input.organizationId, leagueId: operation.leagueId, operationId: operation.id, leaseToken: operation.leaseToken }))) {
    await recordPaymentOperationFailedTerminal({ organizationId: input.organizationId, operationId: operation.id, leaseToken: operation.leaseToken, errorClassification: "invalid_request", errorCode: "F4_DISPATCH_CUTOFF_LOST", now });
    return;
  }
  let provider;
