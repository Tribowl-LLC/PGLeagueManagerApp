...
import {
  ...
  safeAccountGuidanceDeliveryCustomArgs,
  type EmailDispatchResult,
  type EmailSendOptions,
} from './email-core';

export type AccountGuidanceEmailOrganization = {
  name?: string | null;
  slug?: string | null;
  subdomain?: string | null;
};

/** Built-in account-status guidance email; it never contains a bearer token. */
export async function sendAccountGuidanceEmail(options: {
  toEmail: string;
  userName: string;
  noticeType: "account_exists" | "account_missing";
  organization: AccountGuidanceEmailOrganization | null;
  guidanceJobId: number;
}): Promise<EmailDispatchResult> {
  if (!SENDGRID_API_KEY) {
    log.error('Cannot send account guidance email — SENDGRID_API_KEY not configured');
    return { accepted: false, failureReason: "not_configured" };
  }

  const baseUrl = getBaseUrl(options.organization);
  const action = options.noticeType === "account_exists"
    ? {
      subject: "You already have a LeagueVault account",
      heading: "You already have a LeagueVault account",
      intro: "This email address is already connected to a LeagueVault login account.",
      primaryLabel: "Reset password",
      primaryUrl: `${baseUrl}/forgot-password`,
      secondaryLabel: "Log in",
      secondaryUrl: `${baseUrl}/login`,
      closing: "If you remember your password, you can log in directly.",
    }
    : {
      subject: "There is no LeagueVault login account for this email address yet",
      heading: "There is no LeagueVault login account for this email address yet",
      intro: "This email address does not have a LeagueVault login account yet.",
      primaryLabel: "Register",
      primaryUrl: `${baseUrl}/sign-up`,
      secondaryLabel: "",
      secondaryUrl: "",
  ...
  const safePrimaryLabel = escapeHtml(action.primaryLabel);
  const safePrimaryUrl = escapeHtml(action.primaryUrl);
  const safeClosing = escapeHtml(action.closing);
  const safeSecondaryUrl = escapeHtml(action.secondaryUrl);
  ...
  const msg = {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="text-align: center; margin-bottom: 30px;"><h1 style="color: #1a1a2e; margin: 0;">LeagueVault</h1></div>
        <p style="font-size: 16px; color: #333;">Hi ${safeName},</p>
        <h2 style="font-size: 20px; color: #1a1a2e;">${safeHeading}</h2>
        <p style="font-size: 16px; color: #333;">${safeIntro}</p>
        <div style="text-align: center; margin: 30px 0;">
          <a href="${safePrimaryUrl}" style="background-color: #1a1a2e; color: #ffffff; padding: 14px 28px; text-decoration: none; border-radius: 6px; font-size: 16px; display: inline-block; font-weight: bold;">${safePrimaryLabel}</a>
        </div>
        <p style="font-size: 14px; color: #666;">${safeClosing}</p>
        ${secondary}
        <p style="font-size: 14px; color: #666; word-break: break-all;">If the button does not work, copy and paste this link: <a href="${safePrimaryUrl}">${safePrimaryUrl}</a></p>
  ...
  try {
    ...
    if (customArgs) log.info("Account guidance email sent", customArgs);
    return result;
  } catch (error) {
    log.error("Failed to send account guidance email:", describeEmailDeliveryError(error));
