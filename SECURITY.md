...
Slug and subdomain columns are each unique, but uniqueness is not enforced
across the two columns. If one organization's subdomain equals another
organization's slug, the subdomain owner deterministically wins that hostname.
This collision risk and the resolver's cache behavior are tracked in the
limitations below. Once an organization is resolved, the organization-host
...
| Shared rate-limit storage is selected only when `NODE_ENV=production`. | All 19 current `express-rate-limit` instances use `createSharedRateLimitStore`, and the required Render configuration sets `NODE_ENV=production`; their authentication, recovery, setup, payment, administration, invitation, search, public-registration, and disabled Square-webhook buckets are therefore PostgreSQL-backed in production. The process-local store is intentional in development and test. | A multi-process deployment started with another `NODE_ENV` value would silently receive one quota per process, multiplying effective limits on security-sensitive and financially expensive routes. Keep the production setting explicit and verified. A future hardening change should make a production application environment fail at boot if shared rate-limit storage is not active. |
| Organization slugs and subdomains are unique only within their own columns, not across both hostname namespaces. | Resolution is deterministic: an exact subdomain match wins before an exact slug fallback, and the organization-host session guard logs out an authenticated user who does not belong to the resolved tenant before request handling continues. | One tenant's subdomain can shadow another tenant's slug, directing unauthenticated and public tenant-context requests to the subdomain owner despite the slug owner's expectation. [Issue #80](https://github.com/Tribowl-LLC/PGLeagueManagerApp/issues/80) tracks cross-field checks for create and update, the existing-data audit, both collision directions, and a database-safe invariant. |
| Organization hostname results, including misses, are cached in each application process for five minutes and organization mutations do not invalidate that cache. | The cache is bounded by a short TTL; authorization and the organization-host session guard still run against the organization object selected from the cache. | A renamed, deleted, or reassigned label can resolve differently across processes or remain mapped to its previous result until expiry. Add mutation-time invalidation or a shared/versioned cache before hostname reassignment is treated as immediate; use a deliberate transition procedure meanwhile. |
