...
| Organization hostname results, including misses, are cached in each application process for five minutes. | Successful organization creation and hostname updates invalidate the affected identifiers in the mutating process; the cache is bounded by a short TTL, and authorization plus the organization-host session guard still run against the selected organization. | Other application processes can retain the previous result until expiry. Add shared or versioned invalidation before hostname reassignment is treated as immediately consistent across a multi-process deployment; use a deliberate transition procedure meanwhile. |
