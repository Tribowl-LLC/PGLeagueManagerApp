...
beforeEach(() => {
  csrfFetchMock.mockImplementation((url: string, init?: RequestInit) => {
    if (url.includes("canonical-due-past-due")) {
      return Promise.resolve(new Response(JSON.stringify({ data: { rows: [{ id: "11111111-1111-4111-8111-111111111111", payerBowlerId: 7, amountMinor: 2000, outstandingMinor: 2000, dueAt: null, state: "open" }] } }), { status: 200, headers: { "content-type": "application/json" } }));
    }
    expect(init?.method).toBe("POST");
    return Promise.resolve(new Response(JSON.stringify({ data: { fingerprint: `lvpayquote:v1:${"a".repeat(64)}` } }), { status: 200, headers: { "content-type": "application/json" } }));
  });
});
...
describe("history payment dialog occurrence selector", () => {
  it("allows selection inside the open dialog and submits the bound intent", async () => {
    ...
    expect(csrfFetchMock).toHaveBeenCalledWith("/api/financials/leagues/11/interactive-obligation-quote/2", expect.objectContaining({
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ obligationIds: ["11111111-1111-4111-8111-111111111111"] }),
    }));
  ...
  it("formats due dates in the configured league timezone", async () => {
    csrfFetchMock.mockImplementation((url: string) => Promise.resolve(new Response(JSON.stringify(url.includes("canonical-due-past-due")
      ? { data: { rows: [{ id: "11111111-1111-4111-8111-111111111111", payerBowlerId: 7, amountMinor: 2000, outstandingMinor: 2000, dueAt: "2030-01-01T01:30:00.000Z", state: "open" }] } }
      : { data: { fingerprint: `lvpayquote:v1:${"a".repeat(64)}` } }), { status: 200, headers: { "content-type": "application/json" } })));
    const client = new QueryClient({ defaultOptions: { queries: { retry: false } } });
    render(
      <QueryClientProvider client={client}>
        <InteractiveOccurrenceSelector
