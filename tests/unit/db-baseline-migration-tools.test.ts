...
describe('normalized migration baseline tools', () => {
  it('keeps the exact baseline first and all forward migrations ordered', () => {
    ...
    expect(migrations[37]).toMatchObject({
      idx: 37,
      tag: '0037_refund_allocation_adjustments',
      createdAt: 1788965739621,
      hash: '1f06ebc46d62a36405d93c3a6e4fc0bc7b0d7a8a0658e43c313529a8486aaddc',
    });
    ...
    expect(migrations[43]).toMatchObject({
      idx: 43,
      tag: '0043_email_alert_frequency_volume',
      createdAt: 1789521459246,
      hash: 'd452f9c0296f63148e514ee36d479282330ff8d8dcba85bd62223ae66d831c15',
    });
    expect(migrations[43]?.sql).toContain(
      'email_delivery_alerts_bounce_classification_check',
    );
    expect(migrations[44]).toMatchObject({
      idx: 44,
      tag: '0044_cash_edit_obligation_reopen',
      createdAt: 1789587886181,
    });
    ...
    expect(migrations[46]).toMatchObject({
      idx: 46,
      tag: '0046_account_ready_delivery_queue',
      createdAt: 1789689082378,
