...
async function makeFixture() {
  ...
  const [league] = await db.insert(leagues).values({ name: `F3 league ${suffix}`, organizationId: organization.id, locationId: location.id, seasonStart: "2038-01-01", seasonEnd: "2038-12-31", weekDay: "Sunday", competitionStartTime: "19:00", timezone: "UTC", totalBowlingWeeks: 2, weeklyFee: 500, paymentMode: "weekly", payingLineupSize: 3 }).returning({ id: leagues.id });
