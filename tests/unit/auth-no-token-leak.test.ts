...
describe('POST /api/auth/forgot-password persists a non-secret delivery intent', () => {
  const email = 'forgot-leak@vitest.local';
  const request = (body: unknown = { email }) => fetch(`${baseUrl}/api/auth/forgot-password`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body),
  });

  beforeEach(() => {
    mockGetUserByEmail.mockResolvedValue({
      id: 11, email, name: 'Forgot Leak', password: 'hashed:something',
      organizationId: null, credentialGeneration: 3,
    });
    mockEnqueueReset.mockResolvedValue({ kind: 'enqueued', job: { id: 42 } });
  });

  it('commits an intent before acknowledging and never issues a token in the request', async () => {
    let release!: () => void;
    let entered!: () => void;
    const started = new Promise<void>(resolve => { entered = resolve; });
    mockEnqueueReset.mockImplementationOnce(async () => {
      entered();
      await new Promise<void>(resolve => { release = resolve; });
      return { kind: 'enqueued', job: { id: 42 } };
    });
    let acknowledged = false;
    const pending = request().then(response => { acknowledged = true; return response; });
    await started;
    expect(acknowledged).toBe(false);
    expect(mockNotifyReset).not.toHaveBeenCalled();
    release();
    expect((await pending).status).toBe(200);
    expect(mockEnqueueReset).toHaveBeenCalledWith({
      userId: 11, organizationId: null, credentialGeneration: 3, expiresAt: expect.any(Date),
    });
    expect(mockNotifyReset).toHaveBeenCalledOnce();
    expect(mockIssueAccountAction).not.toHaveBeenCalled();
    expect(mockSendTemplatedEmail).not.toHaveBeenCalled();
    expect(JSON.stringify(captured)).not.toContain(email);
    assertNoSecretLeak([RESET_TOKEN]);
  });

  it('returns an operational failure when durable storage fails, without logging sensitive exception text', async () => {
    mockEnqueueReset.mockRejectedValueOnce(new Error(`synthetic failure ${RESET_TOKEN} ${email}`));
  ...
  it('uses identical public responses for queued, suppressed, and unknown accounts', async () => {
    const queued = await (await request()).json();
    mockEnqueueReset.mockResolvedValueOnce({ kind: 'suppressed', reason: 'at_capacity' });
    const suppressed = await (await request()).json();
    mockGetUserByEmail.mockResolvedValueOnce(undefined);
    const unknown = await (await request()).json();
    expect(suppressed).toEqual(queued);
    expect(unknown).toEqual(queued);
    expect(mockNotifyReset).toHaveBeenCalledOnce();
    expect(mockEnqueueReset).toHaveBeenCalledTimes(2);
    assertNoSecretLeak();
  });

  it('does not enqueue passwordless accounts', async () => {
    mockGetUserByEmail.mockResolvedValueOnce({ id: 11, email, password: null });
    expect((await request()).status).toBe(200);
    expect(mockEnqueueReset).not.toHaveBeenCalled();
  });
