...
beforeEach(() => {
  ...
  mockReadPaymentReceiptProjection.mockImplementation(async ({ paymentId }: { paymentId: number }) => {
    ...
    const row = configured?.rows?.find((candidate: { paymentId: number | null }) => candidate.paymentId === paymentId) ?? {
      ...
      businessDate: payment.createdAt ?? '2038-01-01T00:00:00.000Z',
      ...
      dispute: { present: Boolean(payment.disputeId), amountMinor: 0, disputeId: null, scope: 'transaction', state: null, reviewRequired: Boolean(payment.disputeId) },
      unresolved: false,
      receipt: {},
